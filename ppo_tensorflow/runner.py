...
class Runner(object):
    ...
    def run(self):
        ...
        for _ in range(self.nsteps):
            ...
            prev_insts = self.insts
            (self.obs, self.insts), rewards, self.dones, infos = self.env.step(actions)
            self.insts = [utils.id2str(inst) for inst in self.insts]
            self.insts = [utils.word2idx(inst) for inst in self.insts]
            ...
            if self.dones[0] == False:
                ...
                self.mv_maker.export_ani(self.obs[0], prev_insts[0], rewards[0])
        ...
        last_values = self.model.value(self.obs, self.insts, self.states, self.dones)
        ...
        return (*map(sf01, (mb_obs, mb_insts, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs)),
            mb_states, epinfos)
